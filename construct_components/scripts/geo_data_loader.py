...
def load_monthly_data_general(fname, varname, from_date, to_date, months, slice_lon, slice_lat, level, var_norm = True):
    ...
    if level is not None:
        g.slice_level(level)
        print 'I am slicing levels!'
    g.transform_to_anomalies()
    if var_norm:
        g.normalize_variance()
        print 'I just normalised by variance!'
    ### reason why we find only 71 latitudes (rather than 73) -> poles excluded from dataset
    g.slice_spatial(slice_lon, slice_lat)
    # g.slice_date_range(from_date, to_date)
    if months is not None:
        g.slice_months(months)
        print 'Mmmh I am slicing months!'
    return g
...
#                          to_date = date(2012, 1, 1),
#                          months = None):
#     return load_monthly_data_general('/media/peer/Samsung EvO/data1_fast/NCAR_reanalysis/surface_pressure/monthly/pres.mon.mean.nc', 'pres',
#                                      from_date, to_date, months, None, [-89, 89],
#                                      None)
def load_monthly_slp_all(from_date = date(1948, 1, 1),
                         to_date = date(2012, 1, 1),
                         months = None):
    return load_monthly_data_general('/media/peer/fast_data/results_rpca/xizka/data_in/xizka_ad_last150years.nc', 'temp',
                                     from_date, to_date, months, None, [-89, 89],
                                     None)
