...
def pca_eigvals_gf(d):
    ...
    print d.shape,'prov shape'
    # we need the constructed single spatial dimension to be on axis 0
    d = d.transpose()
    print d.shape,'prov 2'
    #### ok now have a timeseries of grid cell data, here the timeseries for each grid cell is now
    #### axis 1, not 0 any more
    #### call pca_eigvals to calc the actual eigenvalues
